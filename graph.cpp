...
void printlist(listnode *thisN,bool showmark=false){
  if(!thisN)return;
  if(showmark)printf("%d,%d ", thisN->dest,thisN->mark);
  else printf("%d ", thisN->dest);
...
bool check(listnode *graph[],listnode *thisN,int dest){
  if(thisN==NULL)return false;
  if(thisN->mark==true)return false;
  thisN->mark=true;
  if(thisN->dest==dest)return true;
  if( check(graph, graph[thisN->dest],dest) )return true;
  if( check(graph, thisN->next,dest) )return true;
  return false;
}
